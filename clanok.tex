...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{LPWAN_struct.png} 
    % https://lucid.app/lucidchart/7dabd3c7-364f-4749-82d4-a4e6ec7698b4/view?page=0_0#
    % \vspace{3cm}
...
\begin{longtable}{l c c c c}
    \caption{Porovnávacia Matica Parametrov LPWAN Technológií \cite{5gtechworld2024_lpwan}}
...
\begin{table}[htbp]
    \centering
    \caption{Kľúčové Parametre Wi-Fi HaLow vs. Ostatné LPWAN \cite{velosiot2023_lpwancases}}
    ...
    \begin{tabular}{l l p{5cm}}
        ...
        Pozícia & ``Most'' medzi Wi-Fi a LPWAN & Optimálna rovnováha rýchlosť $\leftrightarrow$ energia   \\
        \bottomrule
    \end{tabular}
\end{table}
...
Na základe hĺbkovej analýzy teoretických vlastností technológie \textbf{Wi-Fi HaLow} a jej komparácie s konkurenčnými LPWAN štandardmi (kapitola \ref{sec:lpwan_vs_others}) bol formulovaný návrh experimentálneho výskumu. Primárnym cieľom nie je iba samotná implementácia technológie, ale predovšetkým \textbf{empirická verifikácia jej prenosových parametrov} v špecifických podmienkach, ktoré nie sú dostatočne pokryté v existujúcej odbornej literatúre.

\subsection{Vedecké ciele}
Hlavným vedeckým cieľom je kvantifikovať vplyv \textbf{fyzických prekážok} a \textbf{elektromagnetického rušenia} na priepustnosť a latenciu siete štandardu IEEE 802.11ah v priamom porovnaní s technológiou \textbf{LoRaWAN}.

\subsection{Metodika výskumu}
Experimentálny výskum bude realizovaný v dvoch nadväzujúcich fázach, ktoré kombinujú simulačné modelovanie s fyzickým meraním na reálnom hardvéri (testbed).

\subsubsection*{Fáza 1: Simulačné modelovanie}
V prvej fáze bude vytvorený topologický model siete v prostredí \textbf{NS-3 (Network Simulator 3)} s implementovaným modulom pre 802.11ah.
\begin{itemize}
    \item \textbf{Simulačný scenár:} Hviezdicová topológia (Star Topology) s jedným prístupovým bodom (AP) a variabilným počtom koncových staníc (STA) v rozsahu \textbf{10 až 1000 zariadení}.
    \item \textbf{Sledované parametre:} Simulácia sa zameria na \textbf{zaťaženie siete} (Network Load) a analýzu miery stratených paketov (\textbf{Packet Error Rate -- PER}) v dôsledku kolízií.
\end{itemize}

\subsubsection*{Fáza 2: Experimentálne meranie (Testbed)}
Kľúčovým aspektom vedeckej časti je zber presných dát v reálnom čase. Počas experimentov budú zaznamenávané nasledujúce metriky kvality služby (QoS):
\begin{itemize}
    \item \textbf{RSSI (Received Signal Strength Indicator):} Úroveň prijatého signálu [dBm].
    \item \textbf{SNR (Signal-to-Noise Ratio):} Odstup užitočného signálu od šumu [dB].
    \item \textbf{Throughput (Priepustnosť):} Reálna dátová priepustnosť na aplikačnej vrstve, meraná pomocou nástroja \textbf{Iperf}.
    \item \textbf{Latency (Latencia):} Obojsmerné oneskorenie (RTT) pre ICMP pakety.
\end{itemize}

Pre fyzické merania bude zostavené testovacie prostredie pozostávajúce z hardvérových modulov (napr. \textit{Newracom} alebo \textit{Alfa Network}) operujúcich v sub-gigahertzovom frekvenčnom pásme \textbf{868 MHz (EU)}. Merania budú prebiehať v dvoch odlišných prostrediach:

\begin{enumerate}
    \item \textbf{Laboratórne podmienky (Interiér/NLOS):} Testovanie penetrácie signálu cez steny s rôznou materiálovou charakteristikou (tehla, železobetón).
    \item \textbf{Exteriér (Areál STU/LOS):} Meranie maximálneho efektívneho dosahu pri zachovaní minimálnej referenčnej priepustnosti \textbf{150 kbps}.
\end{enumerate}

\subsection{Prínosy a očakávané výsledky}
Pre výskum stanovujeme nasledujúce vedecké otázky a hypotézy:
\begin{itemize}
    \item \textbf{Hypotéza H1:} V prostredí s vysokou hustotou prekážok (železobetónové konštrukcie) vykáže Wi-Fi HaLow pokles priepustnosti o \textbf{menej ako 40\,\%} oproti referenčnému meraniu v priamej viditeľnosti (LOS), zatiaľ čo latencia zostane pod kritickou hranicou \textbf{100 ms}.
    \item \textbf{Hypotéza H2:} Energetická efektivita mechanizmu \textbf{Target Wake Time (TWT)} pri štandarde HaLow prekoná režim spánku LoRaWAN (Trieda A) pri intervaloch odosielania dát kratších ako \textbf{60 sekúnd}.
\end{itemize}

\subsection{Predpokladané výstupy}
\label{sec:vystupy}

Realizácia navrhovaného výskumu prinesie konkrétne výstupy, ktoré presahujú rámec bežnej implementácie:

\begin{enumerate}
    \item \textbf{Komparatívna matica výkonnosti:} Komplexný súbor nameraných dát porovnávajúci HaLow a LoRaWAN v identických podmienkach. Tento výstup slúži na potvrdenie alebo vyvrátenie marketingových tvrdení výrobcov čipov.
    \item \textbf{Model útlmu signálu:} Matematický model popisujúci šírenie signálu 802.11ah v špecifickom prostredí univerzitného kampusu, aplikovateľný pre plánovanie IoT sietí v budovách podobného typu.
    \item \textbf{Odporúčania pre konfiguráciu:} Sada optimálnych parametrov (najmä \textbf{MCS index} a \textbf{Guard Interval}) pre rôzne scenáre nasadenia (napr. statické senzory vs. video stream), overená experimentálne.
\end{enumerate}

Výsledky výskumu budú slúžiť ako podklad pre návrh robustnej senzorovej siete pre monitorovanie environmentálnych veličín v rámci konceptu \textbf{,,Smart Campus``}, pričom poskytnú vedecky podložené argumenty pre výber vhodnej komunikačnej technológie.
